...
if BASICSR_JIT == 'True':
    ...
    print('=> Importing deform_conv_ext')
    try:
        import deform_conv_ext
        print("=> Successfully imported deform_conv_ext.")
    except ImportError:
        print('Import deform_conv_ext error')
        pass
...
class ModulatedDeformConvFunction(Function):
    ...
    def forward(ctx,
        ...
        # Example of moving tensors to the desired device and ensuring dtype
        # Example check for standard layout
        if not input.is_contiguous():
            input = input.contiguous()
        input = input.to(device=device, dtype=torch.float32)
        weight = weight.to(device=device, dtype=torch.float32)
        bias = bias.to(device=device, dtype=torch.float32) if bias is not None else None
        offset = offset.to(device=device, dtype=torch.float32)
        mask = mask.to(device=device, dtype=torch.float32)
        output = output.to(device=device, dtype=torch.float32)  # Assuming output is pre-allocated
        # Repeat for weight, bias, offset, mask, and output tensors
