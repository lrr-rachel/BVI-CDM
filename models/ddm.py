...
class Net(nn.Module):
    ...
    def sample_training(self, x_cond, b, eta=0.):
        ...
        for i, j in zip(reversed(seq), reversed(seq_next)):
            ...
            # estimate clean image at time step t using estimated noise
    ...
    def forward(self, x):
        ...
        # to make sure the noise is sampled from the same distribution as the training data
        # e is the noise
        support_channel_one_side = (self.config.data.supporting_frames - 1) // 2 * 3
        e = torch.randn_like(input_LL_LL[:, support_channel_one_side : (support_channel_one_side + 3), :, :])
        if self.training:
            gt_img_norm = data_transform(x[:, self.config.data.channels:, :, :])
...
class DenoisingDiffusion(object):
    ...
    def train(self, DATASET):
        ...
        for epoch in range(self.start_epoch, self.config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                data_start = time.time()
                if self.config.model.pcd_unet:
                    model_name = "bvi_cdm_model_latest"
                else:
                ...
                if self.step % self.config.training.validation_freq == 0 and self.step != 0:
                    ...
                    utils.logging.save_checkpoint({'step': self.step, 'epoch': epoch + 1,
                                                   ...
                                                   'params': self.args,
                                                   'config': self.config},
                                                  filename=os.path.join(self.config.data.ckpt_dir, model_name))
    ...
    def estimation_loss(self, x, output):
        ...
        gt_img = x[:, self.config.data.channels:, :, :].to(self.device)
        # =============noise loss==================
        ...
        frequency_loss = 0.1 * (self.l2_loss(input_high0, gt_high0) +
                                ...
                                self.l2_loss(pred_LL, gt_LL)) + 0.01 * (self.TV_loss(input_high0) + self.TV_loss(input_high1) + self.TV_loss(pred_LL))
